	...
	<div id="pageAll">
		...
		<div class="page">
			...
			<!-- 	<div class="add">
				...
				<!-- 搜索 -->
				<form>
					<input class="userinput" type="text" placeholder="输入用户名" name="brand_name" />							
				...
				@foreach($data as $v)
						<tr>
							<td>{{$v->brand_id}}</td>
							<td><div class="bsImg">
									<img src="http://www.laravelupd.com/{{$v->brand_img}}" alt="" height="100" width="100">
								</div></td>
							<td>{{$v->brand_name}}</td>
							<td>
							...
							</a> <img class="operation delban" src="/laravel/img/delete.png"></td>
						</tr>
				@endforeach
					</table>
	<div class="paging">{{ $data->appends(['brand_name'=>$brand_name])->links() }}</div>
				</div>
				<!-- banner 表格 显示 end-->
			</div>
			<!-- banner页面样式end -->

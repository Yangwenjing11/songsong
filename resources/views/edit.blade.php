...
<form action="{{url('student/update/'.$data->stu_id)}}" method="post" enctype="multipart/form-data">
    ...
    <p>性别：<input type="radio" name="sex" value="0" @if($data->sex==0) checked="checked" @endif >男
    <input type="radio" name="sex" value="1"  @if($data->sex==1) checked="checked" @endif>女
    </p>
    <input type="hidden" name="oldimg" value="{{$data->headimg}}">
    <p><input type="file" name="headimg"><img src="{{env('UPLOAD_URL')}}{{$data->headimg}}" width="30" heigth="30"></p>
